...
jd_ExportFn jd_UIResult jd_UIBox(jd_UIBoxConfig* config) {
    ...
    b->rect = config->rect;
    jd_V2F min = b->rect.pos;
    jd_V2F max = {b->rect.pos.x + b->rect.size.x, b->rect.pos.y + b->rect.size.y};
    jd_DrawRect(vp->window->renderer, min, max, (jd_V4F){0.8, 0.8, 0.8, 1.0});
...
jd_ExportFn jd_UIViewport* jd_UIBeginViewport(jd_PlatformWindow* window) {
    ...
    jd_TreeLinksClear(vp->root);
    if (vp->root_new->first_child)
        jd_TreeLinkFirstChild(vp->root, vp->root_new->first_child);
