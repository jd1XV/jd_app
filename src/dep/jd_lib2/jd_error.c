...
void jd_ErrorLogFlushToDisk() {
    ...
    for (u32 i = 0; i < _jd_internal_error_log->count; i++) {
        jd_Error* error = &_jd_internal_error_log->errors[i];
        jd_DStringAppend(out, _jd_error_print_indicators[error->severity]);
        jd_DStringAppend(out, error->msg);
        jd_DStringAppend(out, jd_StrLit("\n"));
        jd_DStringAppend(out, jd_StrLit("------------"));
        jd_DStringAppend(out, jd_StrLit("Line: "));
        jd_DStringAppendU32(out, error->line, 10);
    }
