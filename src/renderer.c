...
jd_StrA vs_string = jd_StrAConst("#version 430\n"
...
jd_StrA fs_string = jd_StrAConst("#version 430\n"
                                 "in vec3 frag_uvw; \n"
                                 "in vec4 frag_col; \n"
                                 "out vec4 out_col; \n"
...
void jd_ShaderCreate(jd_Renderer* renderer) {
    ...
    glShaderSource(vs, 1, &vs_string.val, NULL);
    glCompileShader(vs);
    ...
    glShaderSource(fs, 1, &fs_string.val, NULL);
    glCompileShader(fs);
...
jd_Font* jd_FontPushFromFile(jd_Renderer* renderer, jd_StrA ttf_path, s32 face_size_pixels) {
    if (!jd_Platform_FileExists(ttf_path)) return NULL;
    jd_Font* font = jd_ArenaArrayPushBack(renderer->fonts, NULL);
    font->ttf_file = jd_FileReadAll(renderer->arena, ttf_path);
    font->face_size = face_size_pixels;
    jd_File ttf_file = font->ttf_file;
    if (ttf_file.file_size == 0) {
        jd_ArenaArrayPopItem(renderer->fonts, renderer->fonts->count - 1);
    }
    else {
        // setup textures
        ...
        if (stbtt_InitFont(&stb_font, ttf_file.bytes, stbtt_GetFontOffsetForIndex(ttf_file.bytes, 0))) {
...
void jd_RefreshFonts(jd_Renderer* renderer) {
    for (u64 i = 0; i < renderer->fonts->count; i++) {
        jd_Font* font = jd_ArenaArrayGet(renderer->fonts, i);
        glBindTexture(GL_TEXTURE_2D_ARRAY, font->texture);
        stbtt_fontinfo stb_font;
        if (stbtt_InitFont(&stb_font, font->ttf_file.bytes, stbtt_GetFontOffsetForIndex(font->ttf_file.bytes, 0))) {
...
void jd_Internal_DrawGlyph(jd_Renderer* renderer, jd_Font* font, jd_V2F window_pos, jd_V4F col, c8 glyph_char) {
    ...
    jd_ArenaArrayPushBack(renderer->vertices, &top_right);
    jd_ArenaArrayPushBack(renderer->vertices, &bottom_right);
    jd_ArenaArrayPushBack(renderer->vertices, &bottom_left);
    jd_ArenaArrayPushBack(renderer->vertices, &bottom_left);
    jd_ArenaArrayPushBack(renderer->vertices, &top_left);
    jd_ArenaArrayPushBack(renderer->vertices, &top_right);
}

jd_V2F jd_CalcStringBoxMax(jd_Renderer* renderer, jd_Font* font, jd_StrA str, f32 wrap_width) {
...
void jd_DrawString(jd_Renderer* renderer, jd_Font* font, jd_StrA str, jd_V2F window_pos, jd_V4F color, f32 wrap_width) {
...
void jd_DrawStringWithBG(jd_Renderer* renderer, jd_Font* font, jd_StrA str, jd_V2F window_pos, jd_V4F text_color, jd_V4F bg_color, f32 wrap_width) {
...
jd_Renderer* jd_RendererCreate() {
    ...
    renderer->fonts = jd_ArenaArrayCreate(0, 0);
    renderer->default_font = jd_FontPushFromFile(renderer, jd_StrALit("res/libmono.ttf"), 42);
    renderer->active_texture = renderer->default_font->texture;
    renderer->vertices = jd_ArenaArrayCreate(sizeof(jd_GLVertex), MEGABYTES(64));
...
void jd_RendererDraw(jd_Renderer* renderer) {
    glBufferSubData(GL_ARRAY_BUFFER, 0, renderer->vertices->count * sizeof(jd_GLVertex), jd_ArenaArrayGet(renderer->vertices, 0));
    
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D_ARRAY, renderer->active_texture);
    glUseProgram(renderer->objects.shader);
    s32 tex_loc = glGetUniformLocation(renderer->objects.shader, "tex");
    s32 screen_conv_location = glGetUniformLocation(renderer->objects.shader, "screen_conv");
    glUniform1i(tex_loc, 0);
    
    Context* context = ContextGet();
    glUniform2f(screen_conv_location, 2.0f/context->viewport_w, 2.0f/context->viewport_h);
    ...
    jd_ArenaArrayPopTo(renderer->vertices, 0);
}
